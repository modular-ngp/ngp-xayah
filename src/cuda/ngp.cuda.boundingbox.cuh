...
namespace ngp::cuda {
    ...
    inline TCNN_HOST_DEVICE float normdot(const tcnn::vec3& a, const tcnn::vec3& b) {
        float div = tcnn::length(a) * tcnn::length(b);
        if (div == 0.0f) {
            return 1.0f;
        }
    ...
    inline TCNN_HOST_DEVICE float angle(const tcnn::vec3& a, const tcnn::vec3& b) {
        return acosf(tcnn::clamp(normdot(a, b), -1.0f, 1.0f));
    }

    struct Triangle {
        TCNN_HOST_DEVICE tcnn::vec3 sample_uniform_position(const tcnn::vec2& sample) const {
        ...
        TCNN_HOST_DEVICE tcnn::vec3 normal() const {
            return normalize(cross(b - a, c - a));
        }

        TCNN_HOST_DEVICE const tcnn::vec3& operator[](uint32_t i) const {
            return i == 0 ? a : (i == 1 ? b : c);
        }

        TCNN_HOST_DEVICE float angle_at_vertex(uint32_t i) const {
            tcnn::vec3 v1 = (*this)[i] - (*this)[(i + 1) % 3];
            tcnn::vec3 v2 = (*this)[i] - (*this)[(i + 2) % 3];
            return angle(v1, v2);
        }

        TCNN_HOST_DEVICE uint32_t closest_vertex_idx(const tcnn::vec3& pos) const {
            float mag1 = length2(pos - a);
            float mag2 = length2(pos - b);
            float mag3 = length2(pos - c);

            float minv = min(tcnn::vec3{mag1, mag2, mag3});
        ...
        TCNN_HOST_DEVICE float angle_at_pos(const tcnn::vec3& pos) const {
        ...
        TCNN_HOST_DEVICE float ray_intersect(const tcnn::vec3& ro, const tcnn::vec3& rd, tcnn::vec3& n) const {
            tcnn::vec3 v1v0 = b - a;
            tcnn::vec3 v2v0 = c - a;
            tcnn::vec3 rov0 = ro - a;
            n               = cross(v1v0, v2v0);
            tcnn::vec3 q    = cross(rov0, rd);
        ...
        TCNN_HOST_DEVICE float ray_intersect(const tcnn::vec3& ro, const tcnn::vec3& rd) const {
            tcnn::vec3 n;
            return ray_intersect(ro, rd, n);
        }

        // based on https://www.iquilezles.org/www/articles/distfunctions/distfunctions.htm
        TCNN_HOST_DEVICE float distance_sq(const tcnn::vec3& pos) const {
            tcnn::vec3 v21 = b - a;
            tcnn::vec3 p1  = pos - a;
            tcnn::vec3 v32 = c - b;
            tcnn::vec3 p2  = pos - b;
        ...
        TCNN_HOST_DEVICE float distance(const tcnn::vec3& pos) const {
            return sqrt(distance_sq(pos));
        }

        TCNN_HOST_DEVICE bool point_in_triangle(const tcnn::vec3& p) const {
            // Move the triangle so that the point becomes the
            // triangles origin
            tcnn::vec3 local_a = a - p;
            tcnn::vec3 local_b = b - p;
            tcnn::vec3 local_c = c - p;
            ...
            tcnn::vec3 u = cross(local_b, local_c);
            tcnn::vec3 v = cross(local_c, local_a);
            tcnn::vec3 w = cross(local_a, local_b);
        ...
        TCNN_HOST_DEVICE tcnn::vec3 closest_point_to_line(const tcnn::vec3& a, const tcnn::vec3& b, const tcnn::vec3& c) const {
            float t = dot(c - a, b - a) / dot(b - a, b - a);
            t       = max(min(t, 1.0f), 0.0f);
            return a + t * (b - a);
        }

        TCNN_HOST_DEVICE tcnn::vec3 closest_point(tcnn::vec3 point) const {
            ...
            tcnn::vec3 c1 = closest_point_to_line(a, b, point);
            tcnn::vec3 c2 = closest_point_to_line(b, c, point);
            tcnn::vec3 c3 = closest_point_to_line(c, a, point);

            float mag1 = length2(point - c1);
            float mag2 = length2(point - c2);
            float mag3 = length2(point - c3);

            float min = tcnn::min(tcnn::vec3{mag1, mag2, mag3});
        ...
        TCNN_HOST_DEVICE tcnn::vec3 centroid() const {
            return (a + b + c) / 3.0f;
        }
        ...
        TCNN_HOST_DEVICE void get_vertices(tcnn::vec3 v[3]) const {
            v[0] = a;
            v[1] = b;
            v[2] = c;
        }
    ...
    TCNN_HOST_DEVICE inline void project(tcnn::vec3 points[N_POINTS], const tcnn::vec3& axis, float& min, float& max) {
    ...
    struct BoundingBox {
        ...
        TCNN_HOST_DEVICE bool intersects(const Triangle& triangle) const {
            ...
            tcnn::vec3 box_normals[3] = {
                tcnn::vec3{1.0f, 0.0f, 0.0f},
                tcnn::vec3{0.0f, 1.0f, 0.0f},
                tcnn::vec3{0.0f, 0.0f, 1.0f},
            };

            tcnn::vec3 triangle_normal = triangle.normal();
            tcnn::vec3 triangle_verts[3];
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    ...
                    tcnn::vec3 axis = cross(edges[i], box_normals[j]);
        ...
        TCNN_HOST_DEVICE float distance_sq(const tcnn::vec3& p) const {
            return length2(tcnn::max(tcnn::max(min - p, p - max), tcnn::vec3(0.0f)));
        }

        TCNN_HOST_DEVICE float signed_distance(const tcnn::vec3& p) const {
            tcnn::vec3 q = abs(p - min) - diag();
            return length(tcnn::max(q, tcnn::vec3(0.0f))) + std::min(tcnn::max(q), 0.0f);
        }
